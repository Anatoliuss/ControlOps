"""
Загрузка справочника технологических операций из Excel в static_initial_data_control_operations.

Лист "операции+сроки" в Excel — "широкий": одна строка = одна операция, а сроки по регионам
разложены по девяти колонкам. Таблица в БД — "длинная": одна строка на (операция x регион).
Скрипт разворачивает широкий лист в длинный и подставляет идентификаторы из справочников БД.

Формат зависимостей (после перехода на N зависимостей):
    input_operation_order = "23, 17"   input_deadline = "1, 5"
Оба поля пишутся в БД как TEXT без изменений; разбирает их уже C++ (parseInputOperations).
Нулевой элемент списка — основная операция на вход, последующие — альтернативные.

Запуск:
    python scripts/load_init_data.py "инит дата 26 год (новый вариант).xlsx"
    python scripts/load_init_data.py <файл> --dry-run     # только проверки, БД не трогаем
    python scripts/load_init_data.py <файл> --strict      # прерваться на дубликатах

Зависимости: openpyxl, asyncpg, PyYAML (см. scripts/requirements.txt)
Конфиг БД берётся из того же config/db_config.yml, что и C++-программа.
"""
...
# Первая строка листа — технические имена колонок, вторая — русские подписи для людей.
# Данные начинаются с третьей строки.
HEADER_ROW = 1
DATA_START_ROW = 3

# Колонка Excel -> название региона в справочнике sap_regions.
# Идентификаторы не зашиты намеренно: они читаются из БД по названию.
REGION_COLUMNS = {
...
def drop_duplicates(records, strict):
    """Убирает строки, которые нарушают уникальность ключей C++-программы.

    InitialData строит два индекса и падает с runtime_error при дубликате:
      * (culture_id, region_id, order, year)
      * (culture_id, region_id, t_material_id, year, season)
    Регион одинаков для всех развёрнутых строк, поэтому проверяем на уровне Excel.
    """
